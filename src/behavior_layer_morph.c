...
static int on_layer_morph_binding_pressed(struct zmk_behavior_binding *binding,
                                          struct zmk_behavior_binding_event event) {
    const struct device *dev = zmk_behavior_get_binding(binding->behavior_dev);
    const struct behavior_layer_morph_config *cfg = dev->config;

    if (zmk_keymap_layer_active(cfg->layer)) {
        return zmk_behavior_queue_add(event.position, cfg->morphed_binding, true, 0);
    } else {
        return zmk_behavior_queue_add(event.position, cfg->normal_binding, true, 0);
    }
}

static int on_layer_morph_binding_released(struct zmk_behavior_binding *binding,
                                           struct zmk_behavior_binding_event event) {
    const struct device *dev = zmk_behavior_get_binding(binding->behavior_dev);
    const struct behavior_layer_morph_config *cfg = dev->config;

    if (zmk_keymap_layer_active(cfg->layer)) {
        return zmk_behavior_queue_add(event.position, cfg->morphed_binding, false, 0);
    } else {
        return zmk_behavior_queue_add(event.position, cfg->normal_binding, false, 0);
    }
}
...
#define LM_INST(i)                                                                                 \
    ...
    DEVICE_DT_INST_DEFINE(i, behavior_layer_morph_init, NULL, NULL,                                 \
                          &behavior_layer_morph_config_##i, APPLICATION,                           \
                          CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &behavior_layer_morph_driver_api);

DT_INST_FOREACH_STATUS_OKAY(LM_INST)

#define ASSERT_BINDING_LEN_2(n)                                                                    \
    BUILD_ASSERT(BINDING_LEN(n) == 2,                                                              \
                 "Layer morph behavior requires two bindings, the normal and the morphed one")
